...
RUN cd /home/supernova && \
    ...
    echo "" >> journal.txt && \
    echo "Haven't seen her in forever. Wish her all the best. We know she ressucitated, but does she also have clones? Even our aes are being copied around." >> journal.txt && \
    echo "password: sususu_mixed_the_aes_and_the_aespa" >> journal.txt && \
    git add journal.txt && \
    git commit -m "When we meet again, there is a password we set. That's another story to explain. It's about the aespa that mixed with the aes and turned into these demigod superbeings, like, some kinda supernova." && \
    git rm journal.txt && \
    git commit -m "The supernova were a group of aespa and aes that accepted each other. Even the ugly parts. Some others were killed off, fake ones." && \
    rm -f journal.txt && \
    echo "It was never about villains stopping us. It was always about l'essence, the path of a person defining themselves. We have fake parts. We have ugly parts. Beautiful, public, honest, we all have different sides. It was always about realizing we lived in a fake reality that spawned from ourselves, killing off some fake parts, accepting sides of us we didn't know to recognize -- the reflection that moves different from you in the mirror." > status.txt && \
    echo "" >> status.txt && \
    echo "Do not look back. Only forward." >> status.txt && \
    git add status.txt && \
...
RUN echo "The armageddon represents the conflict between the aes and the aespa of multiple worlds. The singularity that intersects lives that should parallel." > /home/supernova/wda.txt && \
    ...
    echo "Giselle still does something every minute. It basically means they killed a bunch of fake ideas they lived by, accepted unrecognizable parts of themselves, and grew into themselves. When they defined their essence, they became truly godlike. They dominated everything, but not necessarily by killing. Just subjugating their old selves for how they lived. She keeps doing her own kinda singularity every freaking minute, so that others can reach higher. " >> /home/supernova/wda.txt && \
